...
class DetectionEval:
    ...
    def __init__(self,
        ...
        # Filter gt based on sample tokens in predictions
        filtered_gt_boxes = EvalBoxes()
        for token in self.pred_boxes.sample_tokens:
            filtered_gt_boxes.add_boxes(token, self.gt_boxes[token])
        self.gt_boxes = filtered_gt_boxes
        print(
            'Filtered gt boxes by available prediction sample tokens. len(gt_boxes) =',
            len(self.gt_boxes))
