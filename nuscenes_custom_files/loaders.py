...
def load_gt(nusc: NuScenes,
    ...
    if eval_split in {
            'train', 'val', 'val_subset', 'val_subset_shifted',
            'val_subset_mini', 'train_detect', 'train_track'
    }:
...
def _get_box_class_field(eval_boxes: EvalBoxes) -> str:
    """Retrieve the name of the class field in the boxes. This parses through
    all boxes until it finds a valid box. If there are no valid boxes, this
