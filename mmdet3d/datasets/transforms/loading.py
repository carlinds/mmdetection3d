...
@TRANSFORMS.register_module()
class LoadMultiViewImageFromFilesSwitchRoot(BaseTransform):
    """Load multi channel images from a list of separate channel files.
    ...
    Args:
        ...
        test_mode (bool): Whether is test mode in loading. Defaults to False.
        set_default_scale (bool): Whether to set default scale.
        data_root_switch (tuple[str, str]): A tuple of two data roots. First one is the original data root, second one is the new data root. Defaults to ("", "").
        data_root_switch_p (float): The probability to use the new data root. Defaults to 0.5.
            Defaults to True.
    """
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if "img_filename" in results:
            for i, f in enumerate(results["img_filename"]):
                if np.random.rand() < self.data_root_switch_p:
                    new_f = f.replace(self.data_root_switch[0], self.data_root_switch[1])
                    if os.path.exists(new_f):
                        f = new_f
                results["img_filename"][i] = f
